...
// === TEAM COLORS ===
const teamColors = [
  "#ff4466",  // Team Glend - red
  "#ffaa33",  // Team Bona - orange
  "#00cc66",  // Team Arief - green
  "#ffcc00",  // Team Irfan - yellow
  "#cc66ff",  // Team Deni - purple
];
...
// === SIMPLIFIED WORLD CONTINENT POLYGONS [lat, lng] ===
const worldPolygons = [
  // North America
  ...
  // Indonesia/Malaysia approximate
...
function drawContinents() {
  ...
  worldPolygons.forEach(poly => {
    ...
    ctx.closePath();
    ctx.fillStyle = "rgba(20,40,75,0.65)";
    ctx.fill();
...
function drawConnections() {
  ...
  const connectionPairs = [[0,1],[0,2],[0,3],[1,2],[2,3],[3,4],[1,4],[0,4]];
  ...
  connectionPairs.forEach(([i, j]) => {
    ...
    ctx.strokeStyle = hexAlpha(nodes[i].color, 0.25);
    ctx.lineWidth = 1;
    ctx.setLineDash([4, 6]);
    ctx.beginPath();
    ctx.moveTo(a.x, a.y);
  ...
  packets.forEach(p => {
    ...
    grad.addColorStop(0.7, hexAlpha(p.color, 0.5));
...
function drawNodes() {
  ...
  nodes.forEach(n => {
    ...
    // Outer pulse ring
    const pulseSize = 20 + Math.sin(animFrame * 0.04 + n.num * 1.5) * 5;
    ctx.beginPath();
    ctx.arc(p.x, p.y, pulseSize, 0, Math.PI * 2);
    ctx.fillStyle = hexAlpha(n.color, 0.08);
    ctx.fill();
...
// === MAP CONTROL BUTTONS (via addEventListener, not inline onclick) ===
